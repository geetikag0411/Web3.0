...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and 
//implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function getCompoundInterest(uint256 principle, uint rate, uint time) public pure returns(uint256) {
        ...
        uint256 Principle = principle*1e8;
        uint256 Rate = rate;
        uint256 divider=100;
        while(time>0)
        {   
            time--;
            Principle+=mulDiv(Principle,Rate,divider);
        }
        return Principle/1e8; 
    }
    
    function reqLoan(uint256 principle, uint rate, uint time) public returns(bool correct) 
    {
        uint256 to_Pay = getCompoundInterest(principle, rate, time);
        ...
        // Also emit the Request event after succesfully adding to the mapping, and return true. Return false if adding to the mapping failed (maybe the user entered a float rate, there were overflows and toPay comes to be lesser than principle, etc.
         if(to_Pay>principle){
            loans[msg.sender]+=to_Pay;
            emit Request(msg.sender,principle,rate,time,to_Pay);
            return true;
        }
        else 
            return false;
    ...
    function getOwnerBalance() public view returns(uint256) 
    {
				...
				// hint: how do you access the functions / variables of the parent class in your favorite programming language?
                // It is similar to that in solidity as well!
    ...
    //settleDues returns a bool, true if the dues were settled and false otherwise. Remember to set the the pending loan to 0 after settling the dues.
